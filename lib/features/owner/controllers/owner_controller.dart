...
class OwnerState {
  ...
  OwnerState copyWith({
    ...
    List<ReportModel>? flaggedReports,
    List<ReportModel>? searchResults,
    List<Map<String, dynamic>>? staffList,
    List<Map<String, dynamic>>? staffLocations,
    List<Map<String, dynamic>>? ownerNotifications,
    List<Map<String, dynamic>>? paymentHistory,
    ReportModel? selectedReport,
    Map<String, dynamic>? selectedStaffProfile,
...
class OwnerController extends StateNotifier<OwnerState> {
  ...
  Future<({Map<String, dynamic>? data, String? error})> createOrder(
    ...
    try {
      final user = await AuthStorage.getUser();
      final res =
          await _api.post('/subscription/create-order', data: {'plan': plan});
      ...
      return (
        data: null,
        error: res.data['message'] as String? ?? 'Could not start payment'
      );
    } on DioException catch (e) {
      state = state.copyWith(isLoading: false);
      return (
        data: null,
        error: friendlyErrorMessage(e, fallback: 'Could not start payment')
      );
    } catch (e) {
      state = state.copyWith(isLoading: false);
      return (
    ...
    try {
      ...
      if (res.data['success'] == true) {
        ...
        state = state.copyWith(
          plan: d['plan'] ?? 'free',
          subscriptionExpiresAt:
              d['expiresAt'] != null ? DateTime.parse(d['expiresAt']) : null,
  ...
  Future<void> scheduleDowngrade(String plan) async {
    try {
      final res =
          await _api.post('/subscription/downgrade', data: {'plan': plan});
      if (res.data['success'] == true) {
  ...
  Future<void> loadStaff() async {
    try {
      ...
      if (res.data['success'] == true) {
        state = state.copyWith(
            staffList: List<Map<String, dynamic>>.from(res.data['data'] ?? []));
      }
    } catch (_) {}
  }
  ...
  Future<({bool ok, String message})> addStaff({
    ...
    try {
      ...
      if (!emailSent) {
        return (
          ...
          message: emailError != null && emailError.isNotEmpty
              ? friendlyErrorMessage(emailError,
                  fallback:
      ...
      return (
        ok: false,
        message: friendlyErrorMessage(e, fallback: 'Could not add staff')
      );
    }
  }
  ...
  Future<void> loadStaffProfile(String staffId) async {
    try {
      ...
      if (res.data['success'] == true) {
        final rRes = await _api
            .get('/reports', params: {'staffId': staffId, 'filter': 'month'});
        state = state.copyWith(
          selectedStaffProfile: res.data['data'],
          staffReports: rRes.data['success'] == true
              ? ((rRes.data['data'] is List
                      ? rRes.data['data']
                      : rRes.data['data']['reports']) as List)
  ...
  Future<void> loadFlaggedReports() async {
    ...
    try {
      ...
      if (res.data['success'] == true) {
        ...
        state = state.copyWith(
            isLoading: false,
            flaggedReports: list.map((r) => ReportModel.fromJson(r)).toList());
      }
    } catch (_) {
      state = state.copyWith(isLoading: false);
    }
  ...
  Future<void> searchReports(String query) async {
    try {
      ...
      if (res.data['success'] == true) {
        final list = (res.data['data']['reports'] as List? ?? []);
        state = state.copyWith(
            searchResults: list.map((r) => ReportModel.fromJson(r)).toList());
      }
    } catch (_) {}
  }
  ...
  Future<void> exportReport(String format) async {
    try {
      await _api.post('/reports/export', data: {'format': format});
    } catch (_) {}
  }
  ...
  Future<void> loadOwnerNotifications() async {
    ...
    try {
      ...
      if (res.data['success'] == true) {
        state = state.copyWith(
            isLoading: false,
            ownerNotifications: List<Map<String, dynamic>>.from(
  ...
  Future<void> markOwnerNotificationsRead() async {
    try {
      ...
      final updated =
          state.ownerNotifications.map((n) => {...n, 'isRead': true}).toList();
  ...
  Future<void> broadcastNotification(String title, String body) async {
    try {
      await _api.post('/notifications/broadcast',
  ...
  Future<void> loadAllReports(
    ...
    try {
      ...
      if (res.data['success'] == true) {
        final newReports = (res.data['data']['reports'] as List)
            .map((r) => ReportModel.fromJson(r))
            .toList();
        state = state.copyWith(
          allReports:
              page == 1 ? newReports : [...state.allReports, ...newReports],
          reportPagination:
              Map<String, dynamic>.from(res.data['data']['pagination'] ?? {}),
        );
      }
    } catch (_) {}
  }
  ...
  Future<void> loadCustomers({String? search}) async {
    try {
      ...
      if (res.data['success'] == true) {
        final d = res.data['data'];
        state = state.copyWith(
          customers: (d['customers'] as List)
              .map((c) => CustomerModel.fromJson(c))
              .toList(),
          canAddCustomer: d['canAdd'] == true ||
              (d['limit'] == -1) ||
              ((d['total'] ?? 0) < (d['limit'] ?? 3)),
  ...
  Future<String?> addCustomer(String name, String phone, String address,
      String city, String notes) async {
    try {
  ...
  Future<void> editCustomer(String id, String name, String phone,
      String address, String city, String notes) async {
    try {
